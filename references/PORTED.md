...
|---|---|---|---|
| Inline images and embedded source context across front ends, provider dialects and durable replay | ACP `docs/protocol/v1/content.mdx`; bounded inline data, source trust and explicit compaction notices | `c849ac2f0c3e4a9f69fdd76a1bdbc659e3f5979d` | `rook-core/src/attachments.rs`, `Message.images`, CLI/TUI/browser/ACP; OpenAI, Anthropic and Google wire formats |
| Explicit parameterized run recipes, selected skill/model, bounded settings and output contract | Goose `documentation/docs/guides/recipes/recipe-reference.md`; adapted as local TOML with single-pass substitution, existing skills and no permission widening | `50666ae0b9a51e260b52b7efbab2e4e020346e94` | `rook-core/src/recipes.rs`, `TurnOptions.recipe`, `run --recipe`, `/recipe`, browser controls |
...
| A streamed update an editor could actually read | opencode `95daf90` *restore session options and reasoning boundaries* — theirs tagged thought chunks with the message id rather than the part id, so two thoughts merged into one; ours sent no `messageId` at all, and the envelope around it was flat where `SessionNotification` is two fields with the update nested | source, via `refs advance` | `rook-acp::protocol::update`, `agent_thought_chunk` |
| A shell in front of a command is a carrier like `env` | cline `e4f1df2` *identify shell editions and discourage redundant wrappers* — theirs reads a wrapper as waste; here `bash -c 'rm -rf /'` put the command inside a quoted argument, where a rule anchored to command position does not look, and walked past the one decision nothing can override | source, via `refs advance` | `policy::past_shell`, `carried` |
